...
users.filter(user => {
  ...
  if (gender === 'female') femaleUsers.push(`${firstName} ${lastName}`);
  else maleUsers.push(`${firstName} ${lastName}`)
  });
...
return users
  .filter(user => {
  const {balance} = user;
  const strToNum = +balance.substring(1).replace(',', '');
  if (strToNum > 20000) return users;
})
  .map(user => {
